						  ...
						  uint8_t chunkIdx)
{
	...
	assert(totalSize == ENTRY_SIZE || isVariableLengthType(datatype));
						...
						uint8_t chunkIdx, VerOffset chunkStart)
{
	...
	if(Item::calculateCrc32(reinterpret_cast<const uint8_t*>(data), item.varLength.dataSize) !=
	   item.varLength.dataCrc32) {
		return ESP_ERR_NVS_NOT_FOUND;
	}
...
esp_err_t Page::mLoadEntryTable()
{
	...
	if(mState == PageState::ACTIVE || mState == PageState::FULL || mState == PageState::FREEING) {
		auto rc = mPartition->read_raw(mBaseAddress + ENTRY_TABLE_OFFSET, mEntryTable.data(), mEntryTable.byteSize());
		if(rc != ESP_OK) {
			mState = PageState::INVALID;
			return rc;
		}
						 ...
						 uint8_t chunkIdx, VerOffset chunkStart)
{
	...
	for(size_t i = start; i < end; i = next) {
		...
		if(chunkIdx != CHUNK_ANY && datatype == ItemType::BLOB_DATA && item.chunkIndex != chunkIdx) {
			continue;
		}
		/* Blob-index will match the <ns,key> with blob data.
         * Skip data chunks when searching for blob index*/
		if(datatype == ItemType::BLOB_IDX && item.chunkIndex != CHUNK_ANY) {
			continue;
		}
		/* Match the version for blob-index*/
		if(datatype == ItemType::BLOB_IDX && chunkStart != VerOffset::VER_ANY &&
